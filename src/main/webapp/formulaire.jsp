...
<%--    création du formaulaire--%>
    ...
    <form action="<%=request.getContextPath()%>/formulaireServlet?societe=${param.societe}&&choix=${param.choix}&&raisonSociale=${param.raisonSociale}"
        ...
        <div class="md:col-span-2 ">
            ...
            <input type="text" id="rs" name="rs"
                   ...
                   focus:border-reversoDark sm:text-sm disabled:opacity-75">
        </div>
...
<%--        données en fonction du client ou du prospect--%>
            ...
            <c:when test="${param.societe.equals('prospect')}">
                <div class="md:col-span-1 mt-2">
                    ...
                    <div class="flex space-x-2">
                        <!-- Jour -->
                        <select id="dateProspectDay" name="dateProspectDay"
                                class="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none
                        ...
                        <!-- Mois -->
                        <select id="dateProspectMonth" name="dateProspectMonth"
                                class="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none
                                focus:border-reversoDark sm:text-sm disabled:opacity-75" required="required">
                        ...
                        <!-- Année -->
                        <select id="dateProspectYear" name="dateProspectYear"
                                class="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none
                                focus:border-reversoDark sm:text-sm disabled:opacity-75" required="required">
        ...
        <div class="md:col-span-2 mt-2">
            ...
            <textarea id="commentaire" name="commentaire"
                      class="shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 focus:outline-none
                   focus:border-reversoDark sm:text-sm disabled:opacity-75"><c:out value="${infoCompany.commentaire}"/></textarea>
        </div>
        <input type="hidden" name="csrfToken" value="${sessionScope.csrfToken}">
        <button type="submit" id="valider" value ="Valider"
               class="grid col-span-2 w-full h-16 md:w-1/2 border border-transparent text-reversoLight shadow-sm
               md:text-xl font-bold rounded-md bg-reversoDark hover:text-reversoDark hover:bg-reversoLight
               hover:ring-2 hover:ring-reversoDark place-self-center">
            <div class="place-self-center">Valider</div>
        </button>
    </form>
</main>
